...
*/
 ...
 $ACCESS_PWD='d02335005aeb87b9d3a54341e281acd87dac8a4858604d0ada285bad4682f7978f718059ec407b2c2ccf5703c46053443abc5847f24a59c07c11318b15f7cb9d';
 $ACCESS_SALT='M5CzRbw0peuizAYmheCF3xnu';
...
//constants
 $VERSION='1.8.120510';
 $MAX_ROWS_PER_PAGE=300; #max number of rows in select per one page
...
//strip quotes if they set
 ...
 if ($_REQUEST['login']){
    if (hash_hmac('sha512', $_REQUEST['pwd'], $ACCESS_SALT)!=$ACCESS_PWD){
...
.qnav{width:30px}
#q{font-family:Monaco,"Courier New",monospace}
</style>
...
function q_prev(){
 ...
 if(qcur<x)qcur=x;
 jQuery('#q').val(ls[LSK+qcur]).trigger('autosize.resize');
}
function q_next(){
 var ls=localStorage;
 if(!lschk())return;
...
<textarea id="q" name="q" cols="70" rows="5" style="width:98%"><?php echo $SQLq?></textarea><br>
...
function print_mkpasswd() {
    if(!empty($_REQUEST['pwd'])) {
        $salt = '';
        for($i = 0; $i < 32; $i++)
            $salt .= chr(mt_rand(40, 122));
        $hash = hash_hmac('sha512', $_REQUEST['pwd'], $salt);
    }
    print_header();
    ?>
    <center>
    <h3>Generate a new password</h3>
    <div style="width:400px;border:1px solid #999999;background-color:#eeeeee;margin-bottom:20px;">
    Password: <input type="text" name="pwd" value="">
    <input type="hidden" name="mkpasswd" value="1">
    <input type="submit" value=" Generate ">
    </div>
    <?php if(isset($hash, $salt)): ?>
    <div style="width:400px;border:1px solid #999999;background-color:#eeeeee;margin-bottom:20px;font-family:Monaco,monospace;text-align:left;word-wrap:break-word;">
    $ACCESS_PWD = '<?php echo $hash; ?>';<br>
    $ACCESS_SALT = '<?php echo $salt; ?>';
    </div>
    <?php endif; ?>
    <?php
    print_footer();
...
<div style="width:400px;border:1px solid #999999;background-color:#eeeeee;margin-bottom:20px;">
<input type="text" name="username" value="IGNORE" style="display:none;">
Password: <input type="password" name="pwd" value="">
<input type="hidden" name="login" value="1">
<input type="submit" value=" Login ">
</div>
<div><a href="?mkpasswd">Generate a new password</a></div>
</center>
<?php
 print_footer();
}
